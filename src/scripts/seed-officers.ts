...
function generateUniqueCode(index: number): string {
  return `MKA-OYO-${String(index + 1).padStart(3, "0")}`;
}

// Map positions to office emails
const positionToOfficeEmail: Record<string, string> = {
...
function getPositionType(position: string): PositionType {
  ...
  if (upperPos.includes("MUHASIB") || upperPos.includes("MURABIY")) {
    return PositionType.SPECIAL;
  }

  // Assistant positions (Naib)
  if (upperPos.startsWith("NAIB")) {
    return PositionType.ASSISTANT;
  }
...
async function seedOfficers() {
  try {
    ...
    const offices = await Office.find({ isDeleted: false });
    ...
    const deleteResult = await Officer.deleteMany({});
    ...
    const officerDocuments = officialOfficers.map((officer, index) => {
      ...
      const officeIds: mongoose.Types.ObjectId[] = [];
    ...
    const insertResult = await Officer.insertMany(officerDocuments);
    ...
    console.log("\n=== OFFICER SUMMARY ===");
    console.log("\nBy Position Type:");
    const executiveCount = insertResult.filter(
      (o) => o.positionType === PositionType.EXECUTIVE
    ).length;
    const headCount = insertResult.filter(
      (o) => o.positionType === PositionType.HEAD
    ).length;
    const assistantCount = insertResult.filter(
      (o) => o.positionType === PositionType.ASSISTANT
    ).length;
    const specialCount = insertResult.filter(
      (o) => o.positionType === PositionType.SPECIAL
    ).length;
    ...
    insertResult.forEach((officer, index) => {
      const typeLabel =
        officer.positionType === PositionType.ASSISTANT ? " [NAIB]" : "";
      console.log(
        `${index + 1}. ${officer.firstName} ${officer.lastName} - ${officer.position}${typeLabel} (${officer.dila})`
      );
    });
